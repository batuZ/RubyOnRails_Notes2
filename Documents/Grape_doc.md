...
注意*both*范围端点必须是`#kind_of？`你的`：type`选项（如果你不提供`：type`选项，它将被猜测等于范围的第一个类终点）。 所以以下内容无效：
...
如果您的应用仅强制执行可用的区域设置，并且：en未包含在您的可用区域设置中，则Grape无法回退到英语并将返回错误消息的翻译密钥。 要避免此行为，请为您的
默认语言环境提供翻译，或者将en添加到您的可用语言环境中。
...
Grape routes 可以在运行时反映出来。 这对于生成文档非常有用。
